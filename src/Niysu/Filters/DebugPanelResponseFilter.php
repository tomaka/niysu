...
class DebugPanelResponseFilter implements \Niysu\HTTPResponseInterface {
	...
	public function __construct(\Niysu\HTTPResponseInterface $response, $phpTemplateService, $scope, $databaseProfilingService) {
	...
	public function appendData($data) {
		...
		// outputting what is before the panel
		$splitOffset = $matches[0][1];
		$this->outputResponse->appendData(substr($data, 0, $splitOffset));
		
		// outputting the panel
		$evaluatedPanel = '';
		$this->phpTemplateService->render(self::$panelTemplate, new \Niysu\Scope([
			...
			'sessionID' => $this->scope->sessionContext->hasSessionLoaded() ? $this->scope->sessionContext->getSessionID() : null,
			'sessionVariables' => []
		]), function($data) { $this->outputResponse->appendData($data); });
		
		// outputting what is after the panel
		$this->outputResponse->appendData(substr($data, $splitOffset));
	}
	...
	private static $panelTemplate = <<<'EOT'
...
<div style="font-size:initial; font-family:Verdana,sans-serif; color:black; position:fixed; left:0; bottom:0; width:100%; padding:0.5em 1em; background-color:gray; border-top:3px double black;">
	...
	<span style="margin-left:2em;">
		...
		>
			<strong><?= 'PHP '.phpversion().' on '.php_uname('s') ?></strong>
			<div style="display:none; background-color:#eee; border:2px solid black; border-radius:10px; padding:1em 2em; position:absolute; bottom:0.7em; left:-50px; width:600px; max-height:40em; overflow-y:auto;">
				<p style="font-weight:bold;"><?= 'PHP '.phpversion().' on '.php_uname('s') ?></p>
				<p><strong>Extensions list</strong>
				<?php $el = get_loaded_extensions(); sort($el, SORT_STRING | SORT_FLAG_CASE); foreach($el as $e): ?>
					<br /><?= $e ?>
				<?php endforeach; ?>
				</p>
			</div>
	...
	<span style="margin-left:2em;">
		...
		>
			...
			<div style="display:none; background-color:#eee; border:2px solid black; border-radius:10px; padding:1em 2em; position:absolute; bottom:0.7em; left:-50px; width:600px; max-height:40em; overflow-y:auto;">
				...
				<?php foreach ($sessionVariables as $key => $val): ?>
